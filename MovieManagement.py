...
class MovieManagement:
    ...
    def addMovie(self, movieName, courseID, description , userID, url, thumbnail64, tag1, tag2, tag3, style):
        ...
        sqlstmt = "INSERT INTO movies (moviename, moviecourseID, professorID, description, url, tag1, tag2, tag3, style, views, likes, thumbnail) VALUES ('"+str(movieName)+"', '"+str(courseID)+"', '"+str(userID)+"', '"+str(description)+"', '"+str(url)+"','"+str(tag1)+"','"+str(tag2)+"','"+str(tag3)+"','"+str(style)+"','"+str(0)+"', '"+str(0)+"',  %s);"      
    ...
    def getLikes(self, movieID):
        sqlstmt = "SELECT likes FROM movies WHERE movieID = '"+str(movieID)+"'"
        df = pd.read_sql_query(sqlstmt, self.database_connection)
        df = df.values.tolist()
    ...
    def addLike(self, movieID, userID):
        ...
        sqlstmt2 = "UPDATE movies SET likes = likes +1 WHERE movieID = '" +str(movieID) +"';"
        try:
            self.database_connection.execute(sqlstmt)
    ...
    def getLikeStatus(self, movieID, userID):
        sqlstmt = "SELECT likeID FROM userlikes WHERE movieID = '"+str(movieID)+"' and userID = '"+str(userID)+"';"
        df = pd.read_sql_query(sqlstmt, self.database_connection)
        df = df.values.tolist()    
        if(len(df) > 0):
            return 1
